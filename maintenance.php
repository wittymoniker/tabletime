...
function tt_delete_post(mysqli $db,int $id,bool $tombstone=true): array {
    ...
    $d=$db->prepare('DELETE FROM post_ratings WHERE post_id=?');if($d){$d->bind_param('i',$id);@$d->execute();$d->close();}
...
function tt_cleanup_orphans(mysqli $db): int {
    $n=0;$db->query('DELETE pr FROM post_ratings pr LEFT JOIN posts p ON p.id=pr.post_id WHERE p.id IS NULL');$n+=$db->affected_rows;$db->query('DELETE pt FROM post_tags pt LEFT JOIN posts p ON p.id=pt.post_id WHERE p.id IS NULL');$n+=$db->affected_rows;$db->query('DELETE ti FROM tag_index ti LEFT JOIN posts p ON p.id=ti.post_id WHERE p.id IS NULL');$n+=$db->affected_rows;$db->query("DELETE FROM federation_tombstones WHERE deleted_at < (UTC_TIMESTAMP() - INTERVAL 180 DAY)");$n+=$db->affected_rows;return $n;
}
